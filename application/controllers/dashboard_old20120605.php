...
class Dashboard extends CI_Controller
{
		...
		else
		{
			...
			if ( $this->orca_auth->user->client_id == 0 || $sms_free > 0 || ( $is_active && ( $sms_quota == 0 || $sms_count < $sms_quota ) ) )
			{
				...
				foreach( $msisdn as $m )
				{
					...
					$this->db->query("INSERT INTO smslog ( to_number, body_plain, campaign_id, sms_number, total_count, customer_id, client_id, queue_id ) 
									  VALUES ( ?, ?, 0, 0, 0, 0, ?, ? )", 
	...
	public function wgpenyiar()
    {
        ...
        $jam = date('H:i:00',strtotime('-7 hours',strtotime("$tanggal $jam")));
        $j = $this->db->escape($jam);
        
        $jamto = date('H:i:59',strtotime('-7 hours',strtotime("$tanggal $jamto")));
        $j1 = $this->db->escape($jamto);
        
        $tanggal = date('Y-m-d',strtotime('-7 hours',strtotime("$tanggal ".date('H:i:00'))));
        ...
        foreach($result as $idx=>$item)
        {
            ...
            $dttime = strtotime('+7 hours', $result[$idx]['timestamp']);
